...
enum PlacementType: CaseIterable {
    ...
    class WordSearch {
        ...
        var labels = [[Label]]() //two-dim array
        ...
        func buildGrid() {
            //create the two-dimensional array, array of Labels
...
//            return false
//            or
//  use Contains .. Where for Sequences
            ...
            return renderer.pdfData { ctx in
                for _ in 0 ..< numberOfPages {
                    ...
                    for i in 0 ... gridSize {
                        ...
                        ctx.cgContext.addLine(to: CGPoint(x: margin + (CGFloat(gridSize) *
                            gridCellSize), y: margin + linePosition))
                        
                        ctx.cgContext.move(to: CGPoint(x: margin + linePosition, y: margin))
                        ctx.cgContext.addLine(to: CGPoint(x: margin + linePosition, y:
                        margin + (CGFloat(gridSize) * gridCellSize)))
                    }
